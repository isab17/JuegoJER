...
class Partidas extends Phaser.Scene {
    ...
    mostrarInputCodigo() {
        ...
        // Fondo popup
        const fondoPopup = this.add.rectangle(config.width / 2, config.height / 2, 400, 250, 0x8b0a82, 0.9)
            .setStrokeStyle(4, 0xffffff)
            .setOrigin(0.5)
            .setDepth(1);
    
        const texto = this.add.text(config.width / 2, config.height / 2 - 90, "Introduce el código de sala:", {
        ...
        inputHTML.className = 'input-field'; // Usa tu CSS
        inputHTML.style.width = '250px';
        inputHTML.style.maxWidth = '90%';
        inputHTML.style.height = '40px';
        inputHTML.style.borderRadius = '10px';
    
        const inputCodigo = this.add.dom(config.width / 2, config.height / 2 - 20, inputHTML)
            .setOrigin(0.5)
            .setDepth(2);
    
        // Crear el botón HTML que faltaba
        const botonHTML = document.createElement('button');
        botonHTML.innerText = 'Aceptar';
        botonHTML.className = 'button-field'; // Usa tu CSS o estilos en línea
        botonHTML.style.width = '150px';
        botonHTML.style.height = '40px';
        botonHTML.style.borderRadius = '10px';
        botonHTML.style.backgroundColor = '#ffffff';
        botonHTML.style.color = '#8b0a82';
        botonHTML.style.fontWeight = 'bold';
        botonHTML.style.fontSize = '16px';
        botonHTML.style.border = 'none';
        botonHTML.style.cursor = 'pointer';
    
        const botonAceptar = this.add.dom(config.width / 2, config.height / 2 + 80, botonHTML)
            .setOrigin(0.5)
            .setDepth(2);
    
        botonAceptar.addListener('click');
        botonAceptar.on('click', () => {
            ...
            if (codigoIngresado.length > 0) {
                ...
                botonAceptar.removeInteractive();
                botonAceptar.node.disabled = true;
                botonAceptar.node.innerText = "Esperando...";
